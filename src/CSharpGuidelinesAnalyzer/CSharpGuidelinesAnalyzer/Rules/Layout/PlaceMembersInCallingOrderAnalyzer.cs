...
namespace CSharpGuidelinesAnalyzer.Rules.Layout
{
    ...
    public sealed class PlaceMembersInCallingOrderAnalyzer : DiagnosticAnalyzer
    {
        ...
        {
            SyntaxNode syntaxRoot = context.SemanticModel.SyntaxTree.GetRoot();

            IEnumerable<FileMember> treeInSourceOrder = scanner.ScanFile(syntaxRoot, context.SemanticModel);
            ...
            SemanticModelAnalysisContext context)
        {
            ...
            string memberLines = Environment.NewLine + string.Join(Environment.NewLine, membersInSortedOrder.Select(item => item.SymbolText));
            var diagnostic = Diagnostic.Create(Rule, member.GetNameToken().GetLocation(), kind, member.SymbolText, memberLines);
        ...
        private sealed class FileScanner
        {
            [NotNull]
            [ItemNotNull]
            public IEnumerable<FileMember> ScanFile([NotNull] SyntaxNode syntaxRoot, [NotNull] SemanticModel semanticModel)
            {
                ...
                {
                    yield return ScanType(typeSyntax, semanticModel, factory);
                }
            }
                ...
                foreach (ISymbol memberSymbol in memberSymbols)
                {
                    foreach (MemberDeclarationSyntax memberSyntax in MemberToSyntax(memberSymbol, semanticModel.SyntaxTree, typeSyntax.Span))
                    {
                        if (memberSyntax is TypeDeclarationSyntax nestedTypeSyntax)
                        {
                            ...
                            {
                                FileMember nestedType = ScanType(nestedTypeSyntax, semanticModel, factory);
        ...
        private sealed class DependencySorter
        {
            ...
            {
                var pendingStack = new Stack<FileMember>(members.Where(item => !item.Usages.Any()).Reverse());
            ...
            private static IEnumerable<FileMember> GroupByType([NotNull] [ItemNotNull] IList<FileMember> members)
            {
                ...
                IEnumerable<TypeDefinition> uniqueTypes = membersByContainingType
                    ...
                    .Concat(types.Select(tuple => tuple.Key))
                    .Distinct();
